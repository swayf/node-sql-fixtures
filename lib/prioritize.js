...
function getNeedsObject(obj) {
  var values = _.reduce(_.values(obj), function(result, value) {
    if (_.isArray(value)) {
  ...
  return values.filter(function(property) {
    return _.isString(property) && property.indexOf(':') > -1 && property.indexOf('::') === -1;
  }).reduce(function(output, property) {

    if (property.includes('{{') && property.includes('}}')) {
      while((match = regexDbl.exec(property)) !== null) {
        output.push(match[1].split(':'));
      }
    } else {
      output.push(property.split(':'));
    }
    return output;
  }, []);
}
...
function satisfied(entry, availablePreReqs) {
  ...
  return _.every(needs, function(need) {
    var preReq = availablePreReqs[need[0]];
    if (preReq) {
      return _.includes(preReq, need[1]);
    }
  });
}
...
module.exports = function prioritize(config) {
  ...
  while (Object.keys(config).length) {
    ...
    if (_.isEmpty(levelEntries)) {
      return new Error("Non-existant dependency. Remaining needs: " + getRemainingNeeds(config).slice(0, 10).join("\n"));
    }
